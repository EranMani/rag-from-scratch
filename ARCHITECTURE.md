...
- **Key data flows added (C45.4):** `question_simplified: bool` field in `AgentState`; `assess_node` checks for difficulty signal before evaluating the answer — step 2 fires the `simplification_prompt` LLM call (once only, guarded by `question_simplified`) and returns a rephrased question; step 3 (second signal) clears `pending_test_question` and adds the slug to `identified_gaps` so `generate_node` falls back to RAG retrieval naturally. `build_selection_result` always resets `question_simplified=False` when a new question is delivered.
